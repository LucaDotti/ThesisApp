...
public class PWBSurveyHandler implements SurveyHandler{
    ...
    public PWBSurveyHandler(Context context) {
    ...
    private String getMissingTime(int surveyId) {
        SurveyConfig config = SurveyConfigFactory.getConfig(Surveys.PWB, context);
        Cursor survey = localController.rawQuery("SELECT * FROM " + PWBTable.TABLE_PWB + " WHERE " + PWBTable.KEY_PWB_ID + " = " + surveyId, null);
        survey.moveToNext();
    ...
    private void createNotification(int surveyId) {
        ...
        remoteViews.setTextViewText(R.id.notificationContent, "New weekly PWB survey available");
